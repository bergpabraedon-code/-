...
- API 入口可配置：前端展示列表和服务端允许访问的上游地址都支持通过环境变量自定义。
...
首次登录后系统会要求重置密码。生产环境强烈建议通过环境变量配置初始管理员：
...
项目使用 Vite + React + TypeScript，默认端口是 `8877`，也支持通过环境变量改成你自己的端口。
...
```bash
VITE_ALLOWED_API_ENDPOINTS=[{"value":"https://www.taijiai.online/","label":"主服务","description":"你的主服务地址"}]
ALLOWED_API_BASE_URLS=https://www.taijiai.online/
...
- `https://www.meitujingling.cn`
  这个域名给前端站点和内置 API 使用。
- `https://www.taijiai.online`
  这个域名给上游 AI 图片接口使用；如果你的前后端共域，也可以两个值填同一个域名。

推荐落地步骤：

1. Fork 或复制这个仓库到你自己的代码仓库。
2. 按 `.env.example` 创建 `.env`，填上你自己的域名、管理员账号和密码。
3. 确认 `VITE_ALLOWED_API_ENDPOINTS` 和 `ALLOWED_API_BASE_URLS` 都已经换成你自己的上游地址。
4. 本地执行 `npm install`、`npm run build`，确认可以正常打包。
5. 把生成后的站点部署到你的服务器或平台，并反向代理到项目运行端口。
6. 将 `PUBLIC_REFERENCE_BASE_URL` 配成最终对外访问的站点地址，否则广场中的引用图地址会指向旧域名。

如果你准备用 Nginx、宝塔、Vercel 或 Docker 部署，这个项目都可以继续落地；只是它带有本地 JSON 存储和 Vite 中间件 API，更适合跑在你自己的服务器或容器里，而不是纯静态托管。
